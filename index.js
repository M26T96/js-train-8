...
function findElementIndexes(arr, element) {
  if (Array.isArray(arr)){
    ...
    m=[...m, arr.indexOf(element) , arr.lastIndexOf(element)]
    return m
  }
  else {
    return []
...
function copyAndSwapElements(arr, target, start, end) {
  if (arr.length !==[]){
    let c = arr.copyWithin(target,start,end)
    return c
  }
  else {
...
function customUnshift(arr, ...elements) {
  if (Array.isArray(arr)){
    ...
    for (let l=initialLength-1; l>=0; l--){
